...
import flightsFakerFactory from "./flightsFakerFactory";
...
const FlightsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = flightsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("flights").create(elem));
...
export default connect(mapState, mapDispatch)(FlightsFakerDialogComponent);
